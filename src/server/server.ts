...
//   const queryString =
//     'SELECT name, location, description FROM Entries INNER JOIN Users ON Entries.user_id = ($1);';
...
app.get('/auth', authController.auth, (req: Request, res: Response) => {
  res.status(200).json({ login: res.locals.token });
});
